...
function scssTask() {
  return src(files.scssPath, { sourcemaps: true }) // set source and turn on sourcemaps
    ...
    .pipe(postcss([autoprefixer(), cssnano(), comments({ removeAll: true })])) // PostCSS plugins
...
function bsWatchTask() {
  watch(files.htmlPath, includeHTML)
...
exports.default = series(
  parallel(scssTask, jsTask, includeHTML),
  cacheBustTask,
  watchTask
)

// Runs all of the above but also spins up a local Browsersync server
// Run by typing in "gulp start" on the command line
